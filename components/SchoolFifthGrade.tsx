...
const BG_URL = 'https://loneboo-images.s3.eu-south-1.amazonaws.com/gradeschoolarcobalenonewsfive.webp';
...
const SchoolFifthGrade: React.FC<{ setView: (view: AppView) => void }> = ({ setView }) => {
    ...
    useEffect(() => {
        if (isLoaded && !isFetching && isAudioOn && audioRef.current) {
            const alreadyHeard = sessionStorage.getItem('heard_audio_school_grade_5') === 'true';
            if (!alreadyHeard) {
                audioRef.current.play().catch(e => console.log("Audio play blocked", e));
                sessionStorage.setItem('heard_audio_school_grade_5', 'true');
            }
        }
        ...
        <div className="fixed inset-0 top-0 left-0 w-full h-[100dvh] z-0 bg-pink-900 overflow-hidden touch-none overscroll-none select-none">
            {(!isLoaded || isFetching) && (
                <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-pink-900/95 backdrop-blur-md">
                    ...
                    <span className="text-white font-black text-lg tracking-widest animate-pulse uppercase flex items-center gap-2">{isFetching ? <Loader2 className="animate-spin" /> : null} Preparo i libri della 5ª...</span>
                </div>
            )}
            ...
            <div className="absolute inset-0 z-0">
                <img src={BG_URL} alt="5ª Elementare" className={`w-full h-full object-fill ${isLoaded ? 'opacity-100' : 'opacity-0'}`} />
            ...
            {isLoaded && !isFetching && !activeSubject && (
                <>
                    ...
                    <button onClick={() => { sessionStorage.setItem('current_diary_grade', '5'); setView(AppView.SCHOOL_DIARY); }} className="absolute bottom-6 right-6 z-50 hover:scale-110 active:scale-95 transition-all outline-none" title="Il mio diario"><img src={BTN_DIARY_IMG} alt="Diario" className="w-32 md:w-48 h-auto drop-shadow-2xl" /></button>
                </>
            )}
