...
const ZONES_MOBILE = [
  ...
  { "id": "lavandino", "label": "Specchio Magico", "points": [{"x":55.17,"y":52.11},{"x":61.57,"y":72.04},{"x":71.7,"y":75.27},{"x":79.16,"y":53.64}] },
  { "id": "cesto", "label": "Atelier di Boo", "points": [{"x":98.61,"y":76.29},{"x":89.29,"y":80.89},{"x":89.55,"y":99.46},{"x":99.41,"y":99.46}] },
...
const ZONES_DESKTOP = [
  ...
  { "id": "lavandino", "label": "Specchio Magico", "points": [{"x":52.93,"y":56.93},{"x":55.43,"y":86.41},{"x":59.54,"y":87.76},{"x":62.55,"y":56.03}] },
  { "id": "shampoo", "label": "Bolle di Sapone", "points": [{"x":50.12,"y":87.31},{"x":49.82,"y":99.01},{"x":53.23,"y":99.01},{"x":53.33,"y":84.83}] },
  { "id": "cesto", "label": "Atelier di Boo", "points": [{"x":65.56,"y":85.28},{"x":65.76,"y":98.11},{"x":74.38,"y":98.11},{"x":74.68,"y":85.28}] }
];
...
const BathroomRoom: React.FC<{ setView: (v: AppView) => void }> = ({ setView }) => {
    ...
    const handleZoneClick = (id: string, label: string) => {
        if (id === 'cesto') {
            // Salviamo il bagno come origine
            sessionStorage.setItem('atelier_origin', AppView.BOO_BATHROOM);
    ...
    if (activePage?.id === 'lavandino') {
        return <MirrorGame onBack={() => setActivePage(null)} />;
    }
