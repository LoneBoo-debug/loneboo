...
// --- BOX DI ESEMPIO PER TEST (PLACEHOLDERS) ---
const MOCK_VIDEOS: Video[] = Array.from({ length: 12 }).map((_, i) => ({
    ...
    category: 'Test',
    url: 'https://www.youtube.com/watch?v=S7Q1CgO6ZQA', // Link di esempio per aprire il modale
...
const VideoGallery: React.FC<VideoGalleryProps> = ({ setView }) => {
  ...
  const [featuredVideo, setFeaturedVideo] = useState<Video | null>(MOCK_VIDEOS[0]); // Fallback immediato su un mock
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedVideo, setSelectedVideo] = useState<Video | null>(null);
  
  // COORDINATE FISSE DELLO SCHERMO (Calibrazione salvata)
  const [calib] = useState({
    top: 9.3,
    left: 18.5,
    width: 62.4,
    height: 20.1
  ...
  const initGallery = useCallback(async () => {
    ...
    try {
        ...
        if (isMounted.current && fetchedVideos.length > 0) {
            setVideos(fetchedVideos.filter(v => v.id !== (fv?.id || '')).slice(0, 15));
        }

    } catch (err) {
        console.error("Gallery UI Error:", err);
        // Fallback garantito se le API falliscono
        if (isMounted.current) setVideos(MOCK_VIDEOS);
    } finally {
    ...
    const delayDebounceFn = setTimeout(async () => {
        if (searchTerm.trim().length > 2) {
            ...
            if (isMounted.current) {
                setVideos(results.length > 0 ? results.slice(0, 12) : MOCK_VIDEOS);
                setLoading(false);
            }
  ...
  return (
    <div className="fixed inset-0 z-0 bg-slate-900 overflow-hidden">
      ...
      <div className="relative z-10 h-full flex flex-col pt-[80px] md:pt-[106px] pb-6 px-3">
        ...
        {/* FEATURED VIDEO - SEMPRE VISIBILE NELLO SCHERMO */}
        {featuredVideo && (
          <div 
            className="absolute z-20 animate-in fade-in duration-700"
            style={{
              top: `${calib.top}%`,
              left: `${calib.left}%`,
              width: `${calib.width}%`,
          ...
          >
            <div 
              className="group relative w-full h-full cursor-pointer bg-black rounded-lg md:rounded-xl shadow-2xl overflow-hidden border-2 border-white/10" 
            ...
            >
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <div className="bg-white/20 backdrop-blur-sm p-3 rounded-full border-4 border-white/40 group-hover:scale-110 transition-transform">
                        <CirclePlay className="text-white w-10 h-10 md:w-16 md:h-16 drop-shadow-2xl" strokeWidth={1.5} />
                    </div>
                </div>
            </div>
          </div>
        ...
        {/* GALLERIA ORIZZONTALE - CENTRATA NEL CINEMA */}
            ...
            <div className="flex overflow-x-auto gap-4 md:gap-8 px-4 py-4 custom-scroll snap-x scroll-smooth no-scrollbar">
                {videos.map((video) => (
                    <div 
                        ...
                        className="flex-shrink-0 w-[200px] md:w-[320px] snap-center group cursor-pointer"
                    >
                        <div className={`relative aspect-video rounded-2xl md:rounded-[35px] border-4 border-black overflow-hidden shadow-2xl transition-all duration-300 group-hover:scale-105 group-hover:-rotate-1 ${video.id.startsWith('mock') ? 'bg-gradient-to-br from-indigo-900 to-purple-800' : 'bg-gray-900'}`}>
                            <img 
                                src={video.thumbnail} 
                                alt="" 
                                className={`w-full h-full object-cover transition-opacity duration-500 ${video.id.startsWith('mock') ? 'opacity-40 grayscale group-hover:opacity-60 group-hover:grayscale-0' : 'opacity-85'}`} 
                                loading="lazy"
                            />
                            
                            {/* Overlay Titolo */}
                            <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                                <div className="bg-white/20 backdrop-blur-sm p-3 rounded-full border-2 border-white/30 transform scale-0 group-hover:scale-100 transition-transform">
                                    <CirclePlay className="text-white w-8 h-8 md:w-12 md:h-12" />
                                </div>
                            </div>

                            <div className="absolute bottom-0 inset-x-0 bg-black/80 backdrop-blur-md px-3 py-2 border-t border-white/10">
                                <h3 className="font-black text-white text-[10px] md:text-sm line-clamp-1 leading-tight uppercase text-center tracking-tight">
                                    {video.title}
                                </h3>
                            </div>
                        </div>
        ...
        {/* BOTTOM SECTION - SEARCH & LINK */}
        <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 mt-auto pb-6 relative z-10">
            <div className="relative w-full max-w-xl">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
                <input 
                  type="text" 
                  placeholder="Cerca nel canale" 
                  value={searchTerm} 
                  onChange={(e) => setSearchTerm(e.target.value)} 
                  className="w-full pl-12 pr-10 py-3 rounded-2xl border-4 border-black font-black text-base text-black focus:outline-none shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] bg-white/95 backdrop-blur-sm" 
                />
            </div>
          ...
          <div className="video-modal-overlay fixed inset-0 z-[200] flex items-center justify-center bg-black/95 p-0 md:p-4 animate-in fade-in" onClick={() => setSelectedVideo(null)}>
             <div className="video-modal-container relative w-full max-w-5xl bg-white rounded-[30px] md:rounded-[40px] border-[6px] md:border-[8px] border-red-600 shadow-2xl overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="iframe-container flex-1 w-full aspect-video bg-black">
                    <iframe 
                      src={`https://www.youtube.com/embed/${selectedVideo.id.startsWith('mock') ? 'S7Q1CgO6ZQA' : selectedVideo.id}?autoplay=1&rel=0&modestbranding=1&iv_load_policy=3`} 
