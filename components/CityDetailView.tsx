...
const CityDetailView: React.FC<CityDetailViewProps> = ({ 
    title, 
    setView, 
    bgColor = "bg-white", 
    bgImage,
    ...
    return (
        <div className={`fixed inset-0 z-0 flex flex-col items-center justify-center overflow-hidden ${bgColor}`}>
            ...
            {/* OVERLAY SPACING */}
            {showConstruction && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-8 text-center bg-black/10 backdrop-blur-[2px]">
            ...
            <button 
                onClick={() => setView(AppView.TRAIN_JOURNEY)}
                className="fixed top-4 right-4 z-50 bg-white w-14 h-14 md:w-20 md:h-20 rounded-full border-4 border-black hover:scale-110 active:scale-95 transition-all flex items-center justify-center text-black shadow-2xl"
            >
