...
type SocialStatsState = { youtube: string; instagram: string; tiktok: string; facebook: string; };
...
const SocialHub: React.FC<{ setView?: (view: AppView) => void }> = ({ setView }) => {
  ...
  const handleExternalClick = (e: React.MouseEvent, id: string) => {
      ...
      if (!social) { social = SUPPORT_LINKS.find(s => s.platform.toLowerCase() === id.toLowerCase()); }
  ...
  const renderInteractives = (isDesktop: boolean) => {
      ...
      <>
        {activeStats.map((loc) => (
            <div key={loc.id} className="absolute flex items-center justify-center z-20 pointer-events-none" style={{ top: loc.top, left: loc.left, width: loc.width, height: loc.height }}>
                <span className={`font-mono font-black leading-none ${loc.color} ${loc.glow}`} style={{ fontSize: isDesktop ? 'min(1.8vw, 2.5vh)' : 'min(4.5vw, 3.5vh)', textShadow: '0 0 4px black' }}>
                    {/* @ts-ignore */}
                    {stats[loc.id]}
                </span>
            </div>
        ...
        {activeZones.map((zone) => (
            <div key={zone.id} onClick={(e: any) => handleExternalClick(e, zone.id)} className="absolute inset-0 cursor-pointer z-30" style={{ clipPath: getClipPath(zone.points) }} title={`Vai a ${zone.id}`} />
        ))}
      </>
      );
  };
    ...
    <div className="fixed inset-0 top-0 left-0 w-full h-[100dvh] z-10 bg-indigo-900 overflow-hidden touch-none overscroll-none select-none">
        ...
        <div className="relative w-full h-full overflow-hidden">
            ...
            <div className="block md:hidden w-full h-full relative">
                <img src={TRAIN_BG_MOBILE} alt="Stazione Lone Boo Mobile" className={`w-full h-full object-fill object-center transition-opacity duration-700 ${bgLoaded ? 'opacity-100' : 'opacity-0'}`} draggable={false} />
                {bgLoaded && renderInteractives(false)}
            </div>
            <div className="hidden md:block w-full h-full relative">
                <img src={TRAIN_BG_DESKTOP} alt="Stazione Lone Boo Desktop" className={`absolute inset-0 w-full h-full object-fill object-center transition-opacity duration-700 ${bgLoaded ? 'opacity-100' : 'opacity-0'}`} draggable={false} />
