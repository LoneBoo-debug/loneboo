...
const LibraryReadView: React.FC<{ setView: (view: AppView) => void }> = ({ setView }) => {
    ...
    return (
        <div className="fixed inset-0 top-0 left-0 w-full h-[100dvh] z-0 bg-[#3e2723] overflow-hidden flex flex-col">
            <style>{`
            ...
            <div className="relative z-10 w-full pt-[80px] md:pt-[110px] px-4 flex flex-col shrink-0">
                <div className="w-full flex justify-between items-center mb-1">
                    <button onClick={() => setView(AppView.BOOKS_LIST)} className="hover:scale-110 active:scale-95 transition-all outline-none">
                ...
                <div className="self-end mr-2 md:mr-10 bg-white/10 backdrop-blur-sm px-4 py-1.5 rounded-2xl border border-white/20 shadow-md">
                    <p className="text-white font-bold text-[8px] md:text-[11px] uppercase tracking-tight text-right italic whitespace-nowrap" style={{ textShadow: '1px 1px 2px black' }}>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-6 max-w-7xl mx-auto">
                    {READING_DATABASE.map((book) => (
                        <div 
                            ...
                            className="group relative bg-white/10 backdrop-blur-sm rounded-2xl border-2 border-white/20 p-1.5 cursor-pointer hover:scale-105 active:scale-95 transition-all shadow-xl flex flex-col h-full"
                        >
                            <div className="aspect-[3/4] w-full rounded-xl overflow-hidden border-2 border-white shadow-md mb-2">
                                <img src={book.thumbnail} alt={book.title} className="w-full h-full object-cover" />
                            </div>
                            <div className="bg-white rounded-lg p-1.5 text-center mt-auto">
            ...
            {selectedBook && (
                <div className="fixed inset-0 z-[200] bg-black/98 backdrop-blur-md flex flex-col animate-in fade-in duration-300">
                    <div className="w-full pt-[74px] md:pt-[106px] px-6 pb-2 flex justify-between items-center shrink-0 border-b border-white/10">
                        <h3 className="text-white font-black text-sm md:text-2xl uppercase truncate drop-shadow-md">{selectedBook.title}</h3>
                        <button onClick={closeBook} className="hover:scale-110 active:scale-95 transition-all">
                            <img src={BTN_CLOSE_IMG} alt="Chiudi" className="w-10 h-10 md:w-14 h-auto" />
                        </button>
                    </div>

                    <div id="book-page-container" className="flex-1 w-full overflow-hidden flex items-center justify-center p-2 md:p-4">
                        <img 
                            key={currentPage}
                            src={selectedBook.pages[currentPage]} 
                            className="max-w-full max-h-full object-contain rounded-lg page-shadow border-2 border-white/10 animate-in slide-in-from-right-4 duration-500" 
                            alt={`Pagina ${currentPage + 1}`}
                        />
                    </div>

                    <div className="w-full h-[90px] md:h-[130px] flex items-center justify-center gap-4 md:gap-12 pb-4 bg-gradient-to-t from-black/90 to-transparent">
                        <button 
                            onClick={prevPage}
                            disabled={currentPage === 0}
                            className={`p-3 md:p-5 rounded-full border-4 border-black transition-all ${currentPage === 0 ? 'bg-gray-500 opacity-30 grayscale' : 'bg-yellow-400 hover:bg-yellow-300 active:translate-y-1'}`}
                        >
                            <ChevronLeft size={28} strokeWidth={4} className="text-black" />
                        </button>

                        <div className="bg-white px-5 py-2 md:px-10 md:py-4 rounded-2xl border-4 border-black flex flex-col items-center">
                            <span className="text-[8px] md:text-sm font-black text-gray-400 uppercase">Pagina</span>
                        ...
                        <button 
                            ...
                            className={`p-3 md:p-5 rounded-full border-4 border-black transition-all ${currentPage === selectedBook.pages.length - 1 ? 'bg-gray-500 opacity-30 grayscale' : 'bg-green-500 hover:bg-green-400 active:translate-y-1'}`}
                        >
