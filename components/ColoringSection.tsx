...
import { COLORING_CATEGORIES } from '../constants';
...
const ColoringSection: React.FC = () => {
  ...
  useEffect(() => {
      ...
      imgMobile.src = SCHOOL_BG_MOBILE;
      const imgDesktop = new Image();
      imgDesktop.src = SCHOOL_BG_DESKTOP;
      ...
      const timer = setTimeout(() => {
          if (!selectedCategory) setShowHint(true);
      }, 5000); 
  ...
  const handleZoneClick = (zoneId: string) => {
      ...
      let category = COLORING_CATEGORIES.find(c => c.id === zoneId);
      if (!category) {
        ...
        {selectedCategory ? (
            <div className="w-full h-full bg-white overflow-y-auto animate-in slide-in-from-right duration-300">
                <div className="max-w-7xl mx-auto p-4 md:p-6 pb-24 min-h-full">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 mt-4 md:mt-8 border-b-4 border-gray-100 pb-6">
                        <button 
                            onClick={() => setSelectedCategory(null)}
                            className="flex items-center gap-2 bg-white text-gray-800 font-black px-6 py-3 rounded-full border-4 border-black shadow-[4px_4px_0px_0px_black] hover:scale-105 active:shadow-none active:translate-y-1 transition-all"
                        >
                            <ArrowLeft size={24} strokeWidth={3} /> TORNA ALL'ACCADEMIA
                        </button>
                        <h2 className={`text-3xl md:text-5xl font-black uppercase text-center ${selectedCategory.color.replace('bg-', 'text-')}`}>
                    ...
                    {selectedCategory.items.length > 0 ? (
                            ...
                            {selectedCategory.items.map((item) => (
                                <div key={item.id} className="bg-white rounded-2xl overflow-hidden border-4 border-gray-200 hover:border-boo-purple transition-colors shadow-lg flex flex-col">
                                    ...
                                    <div className="p-3 flex flex-col flex-1">
                                        <h4 className="font-bold text-gray-700 text-sm md:text-base leading-tight mb-2 line-clamp-2">
                                            {item.title}
                                        </h4>
                                        <div className="mt-auto pt-2">
                                            <a 
                                                ...
                                                download
                                                className={`
                                                    flex items-center justify-center gap-2 w-full py-2 rounded-xl text-white font-black text-sm transition-all shadow-md active:scale-95
                                                    ${selectedCategory.color} hover:brightness-110
                                                `}
                                            >
                        ...
                        <div className="flex flex-col items-center justify-center text-center py-12 bg-white rounded-[40px] border-4 border-dashed border-gray-300 shadow-sm min-h-[50vh] animate-in zoom-in">
                            <div className="w-48 h-48 md:w-64 md:h-64 mb-6 relative">
                                <img src={CONSTRUCTION_IMG} alt="Lavori in corso" className="w-full h-full object-contain drop-shadow-lg" />
                            </div>
                            <h3 className="text-3xl md:text-4xl font-black text-gray-700 mb-4">
                                Lavori in Corso! 🚧
                            </h3>
                            <p className="text-xl font-bold text-gray-500 max-w-md leading-relaxed px-4">
                                Sto disegnando nuovi capolavori per la sezione <span className={`inline-block ${selectedCategory.color} text-white px-3 py-1 rounded-lg mx-1`}>{selectedCategory.title}</span>!
                                <br/><br/>
                                Torna a trovarci presto! 👻✏️
                            </p>
                        </div>
                    )}
                </div>
            ...
            <div 
                className="relative w-full h-full bg-gradient-to-b from-[#b388f4] to-white overflow-hidden flex flex-col"
                onClick={handleInteraction}
            >
                {!isLoaded && (
                    <div className="absolute inset-0 flex items-center justify-center bg-orange-100 z-50">
                        <span className="text-orange-600 font-black text-2xl animate-pulse flex items-center gap-2">
                            <Loader2 className="animate-spin" /> Entro in Accademia...
                        </span>
                    </div>
                )}

                <RobotHint 
                    show={showHint && isLoaded && !selectedCategory} 
                    message="Tocca un disegno, stampalo e... coloriamo!"
                />
                ...
                <div className="relative flex-1 w-full h-full overflow-hidden select-none">
                    ...
                    <div className="hidden md:block w-full h-full relative overflow-hidden">
                        <img 
                            src={SCHOOL_BG_DESKTOP} 
                            alt="Accademia Lone Boo Desktop" 
