...
interface SchoolGymBasketProps {
    setView: (view: AppView) => void;
}

const SchoolGymBasket: React.FC<SchoolGymBasketProps> = ({ setView }) => {
    ...
    return (
        <div className="fixed inset-0 top-0 left-0 w-full h-[100dvh] z-0 bg-[#0f172a] overflow-hidden touch-none overscroll-none select-none flex flex-col">
            ...
            {!isLoaded && (
                <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-slate-900/95 backdrop-blur-md">
                    ...
                    <span className="text-white font-black text-lg tracking-widest animate-pulse uppercase">Vado a canestro...</span>
                </div>
            )}
            ...
            {isLoaded && (
                <>
                    {/* TASTO CHIUDI - SPOSTATO IN ALTO A SINISTRA SOTTO L'HEADER */}
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-end pb-[20vh] pointer-events-none">
                         <div className="animate-in slide-in-from-bottom-10 duration-700">
                             <p 
                                className="text-orange-500 font-luckiest text-3xl md:text-7xl uppercase tracking-widest text-center"
                                style={{ 
                                    WebkitTextStroke: '2px black',
                                    textShadow: '4px 4px 0px black'
                                }}
                             >
                                GIOCO BASKET <br/> IN ARRIVO! 🏀
                             </p>
                        </div>
                    </div>
                </>
