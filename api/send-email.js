...
export default async function handler(req, res) {
  // 1. Permetti solo POST
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Metodo non consentito' });
  }

  try {
    const { name, age, city, province, image } = req.body;
    const apiKey = process.env.RESEND_API_KEY;

    if (!apiKey) {
      console.error("ERRORE: RESEND_API_KEY mancante nelle variabili d'ambiente.");
      return res.status(500).json({ error: 'Configurazione server incompleta' });
    }

    if (!name || !image) {
      return res.status(400).json({ error: 'Nome o immagine mancanti' });
    }

    const resend = new Resend(apiKey);
    
    // Pulizia del contenuto Base64 per l'allegato
    const base64Content = image.includes(',') ? image.split(',')[1] : image;

    // 2. Invio tramite Resend (Destrutturazione corretta { data, error })
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; color: #333; line-height: 1.6; max-width: 600px; margin: 0 auto; border: 4px solid #8B5CF6; padding: 20px; border-radius: 20px;">
          ...
          <p style="text-align: center;">Il capolavoro è allegato a questa email!</p>
    ...
    if (error) {
      console.error("Errore Resend API:", error);
    ...
    console.error('Errore critico durante l\'invio email:', err);
